...
def parse_lc18_raw(_path='galcem/input/yields/sncc/lc18/'):
    yield_path = _path+'tab_R/tab_yieldstot_iso_exp.dec'
    remn_path = _path+'RemnantMass/remnant_masses.txt'
    yield_eps = 1e-13
    inputs = glc.Inputs()
    Zsun = inputs.solar_metallicity
    df_raw = pd.read_fwf(yield_path, infer_rows=400, header=None)
    df_remnant = pd.read_fwf(remn_path)
    ...
    df = pd.DataFrame({'isotope':[],'a':[],'z':[],'yield':[],'mass':[],'metallicity':[],'irv':[],'mass_ej':[],'massfrac':[],'ysign':[]})
    for i in range(len(header_idxs)-1):
        ...
        df_rem_slice = df_r.loc[df_r['v'].eq(_df['irv'])].loc[df_r['model'].eq(zini_remmap[z_ini])]
        ...
        _df.columns = ['isotope','z','a','yield','mass','irv','metallicity','mass_ej','massfrac','ysign']
