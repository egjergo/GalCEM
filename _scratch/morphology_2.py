...
class Initial_Mass_Function:
    ...
    def Salpeter55(self, Mstar, plaw=1.35):
        return Mstar ** (-(1 + plaw))
     
    def Kroupa01(self, Mstar):
        plaw = np.ones(Mstar.size)
        plaw[np.where(Mstar < 0.08)[0]] = -0.7
        plaw[np.where((Mstar >= 0.08) & (Mstar < 0.5))[0]] = 0.3
        plaw[np.where(Mstar >= 0.5)[0]] = 1.3
        return np.power(Mstar, -(1 + plaw))
           
    def Kroupa03(self, Mstar):#, C = 0.31): #if m <= 0.5: lambda m: 0.58 * (m ** -0.30)/m
        '''Kroupa & Weidner (2003)'''
        plaw = np.where(Mstar <=1.0, 1.2,1.7)
        return Mstar ** (-(1 + plaw))
        
    def IMF_select(self):
        if not self.custom:
            return eval("self."+self.option)
        if self.custom:
            return self.custom
    
    def integrand(self, Mstar):
        return self.IMF_select()(Mstar) #Mstar * self.IMF_select()(Mstar)
        
    def normalization(self): 
        Mtot = np.logspace(np.log10(self.Ml), np.log10(self.Mu), num=200)
        return np.reciprocal(scipy.integrate.simps(self.integrand(Mtot), x=Mtot))

    def IMF(self, Mstar): #!!!!!!!! might not be efficient with the IGIMF
        return self.integrand(Mstar) * self.normalization()
